...
"sprites/wall.xpm", &data->sprites.wall.x, &data->sprites.wall.y);
	data->sprites.enemies.img_ptr = mlx_xpm_file_to_image(data->mlx_ptr, \
"sprites/enemies.xpm", &data->sprites.enemies.x, &data->sprites.enemies.y);
	...
	if (data->sprites.enemies.img_ptr == NULL)
		ft_error(data, "missing enemies.xpm file");
}
